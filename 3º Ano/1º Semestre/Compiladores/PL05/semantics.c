...
void check_parameters(struct node *parameters){
    struct symbol_list *param_table = malloc(sizeof(struct symbol_list));
    param_table->next= NULL;

    struct node *param_node = parameters->children->node;

    while(param_node != NULL){

        struct node *parameter = param_node->children->node;
        struct node *id  = getchild(parameter,1);

        if(search_symbol(param_table, id->token) != NULL)
        {
            printf("Identifier %s already declared\n",id->token);
            semantic_errors++;
        }
        else{
            insert_symbol(param_table,id->token,category_type(parameter->category),parameter);
        }

        param_node = param_node->children->node;
    }

    struct symbol_list *current = param_table;
